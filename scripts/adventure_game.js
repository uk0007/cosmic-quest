...
(function(window) {
  ...
  class AdventureLevelScene extends Phaser.Scene {
    ...
    create() {
      ...
      if(cfg.id===1){this.createWoodlandTraps();this.createCannonEncounters();}
    ...
    createCannonEncounters() {
      ...
      this.sweepCutters=[{x:9830,y:g-200,range:95},{x:12960,y:g-210,range:120}].map(cfg=>{
    ...
    updateCannonEncounters(delta) {
      ...
      this.cannonShots.forEach(p=>{
        if(!p.live)return;p.body.reset(p.x-delta*0.32,p.y);
