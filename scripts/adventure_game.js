...
(function(window) {
  ...
  // Extend only the woodland finale; preserve the existing jump distances.
  {
    const cfg=LEVEL_CONFIGS[1], art=window.CosmicIllustratedScenes[1];
    const shift=x=>x>=4800?x+1400:x;
    cfg.levelWidth+=1400;cfg.exitX+=1400;
    ['platformSpots','movingSpots','collapsingRocks','boneOffsets','crystalOffsets','enemies','hazards'].forEach(key=>{
      (cfg[key]||[]).forEach(item=>{['x','minX','maxX'].forEach(k=>{if(typeof item[k]==='number')item[k]=shift(item[k]);});});
    });
    cfg.gateLocations=[800,1820,2400,3200,4100,4700,6660];
    cfg.enemies[4]={...cfg.enemies[4],x:3370,minX:3320,maxX:3450};
    art.sections.forEach(section=>{section.landmark[1]=shift(section.landmark[1]);section.decor.forEach(item=>item[1]=shift(item[1]));});
    ['distant','groundPatches'].forEach(key=>art[key].forEach(item=>item[1]=shift(item[1])));
    art.sections[6].decor.push(['ancient_tree',5120,0.95],['rock_low',5440,0.8],['fork_tree',5720,1.05],['rock_low',6020,0.75]);
    art.groundPatches.push(['ground_4',5230,0.75],['ground_3',5920,0.8]);
    LEVEL_SECTIONS[1][6][2]=art.sections[6].landmark[1];
  }
  // Seven authored extension corridors, one before each existing question gate.
  {
    const cfg=LEVEL_CONFIGS[1],art=window.CosmicIllustratedScenes[1];
    const cuts=cfg.gateLocations.map((x,i)=>({x:x-150,length:i===6?1300:1000}));
    const expand=x=>x+cuts.reduce((sum,c)=>sum+(x>=c.x?c.length:0),0);
    ['platformSpots','movingSpots','collapsingRocks','boneOffsets','crystalOffsets','enemies','hazards','thorns'].forEach(key=>{
      (cfg[key]||[]).forEach(item=>['x','minX','maxX'].forEach(k=>{if(typeof item[k]==='number')item[k]=expand(item[k]);}));
    });
    cfg.trenches.forEach(t=>{t.startX=expand(t.startX);t.endX=expand(t.endX);});
    cfg.gateLocations=cfg.gateLocations.map(expand);cfg.exitX=expand(cfg.exitX);cfg.levelWidth=14600;
    cfg.cutterSpots=[5320,5840].map(expand);
    art.sections.forEach(section=>{section.landmark[1]=expand(section.landmark[1]);section.decor.forEach(item=>item[1]=expand(item[1]));});
    ['distant','groundPatches'].forEach(key=>art[key].forEach(item=>item[1]=expand(item[1])));
    let added=0;
    cuts.forEach((cut,i)=>{
      const start=cut.x+added;added+=cut.length;
      art.sections[i].decor.push(['ancient_tree',start+80,0.9],['fork_tree',start+cut.length-100,0.95],['rock_low',start+360,0.65]);
      art.groundPatches.push(['ground_3',start+260,0.85],['ground_4',start+730,0.7]);
      // Wide solid waiting spaces separate every cutter from the next jump or gate.
      cfg.cutterSpots.push(start+220);
      if([1,3,5].includes(i)){
        cfg.trenches.push({startX:start+480,endX:start+720});
        cfg.movingSpots.push({x:start+600,y:-80,distanceX:55,distanceY:0,duration:2400,scale:0.40});
        cfg.platformSpots.push({x:start+530,y:-65,scale:0.22},{x:start+680,y:-65,scale:0.22});
      }else{
        cfg.thorns.push({x:start+580,y:0,scale:0.75,damage:8});
        cfg.platformSpots.push({x:start+580,y:-115,scale:0.35});
        cfg.movingSpots.push({x:start+760,y:-75,distanceY:-95,duration:2600,scale:0.35});
      }
      LEVEL_SECTIONS[1][i][2]=art.sections[i].landmark[1];
    });
    // Final timing trials: safe staging banks and a brief open crossing window.
    cfg.timingTrials=[
      {startX:10030,endX:10270,x:10170,period:5400,closed:3300,label:'RHYTHM RAVINE'},
      {startX:13200,endX:13500,x:13350,period:5400,closed:3600,label:'FINAL CLOCKWORK CROSSING'}
    ];
    cfg.trenches.push({startX:13200,endX:13500});
    cfg.platformSpots.push({x:13275,y:-75,scale:0.22},{x:13430,y:-75,scale:0.22});
    cfg.enemies.push(
      {type:'ground',x:9680,y:-45,minX:9580,maxX:9790,speed:90},
      {type:'fly',x:9930,y:-165,minX:9850,maxX:10000,speed:85},
      {type:'armored',x:10420,y:-45,minX:10330,maxX:10470,speed:70},
      {type:'ground',x:12670,y:-45,minX:12570,maxX:12770,speed:95},
      {type:'fly',x:13060,y:-190,minX:12960,maxX:13150,speed:90},
      {type:'armored',x:13610,y:-45,minX:13550,maxX:13700,speed:75}
    );
    cfg.platformSpots.push({x:9780,y:-135,scale:0.35},{x:12820,y:-140,scale:0.35});
    cfg.cannonSpots=[
      {x:9970,y:-52,minX:9490,maxX:10010,period:1450},
      {x:10480,y:-165,minX:10290,maxX:10500,period:1850},
      {x:13100,y:-52,minX:12480,maxX:13130,period:1250},
      {x:13700,y:-150,minX:13520,maxX:13720,period:1600}
    ];
    cfg.trenches.sort((a,b)=>a.startX-b.startX);
  }
  // Crystal Caverns: longer chambers with slightly faster encounter rhythms.
  {
    const cfg=LEVEL_CONFIGS[2],art=window.CosmicIllustratedScenes[2];
    const cuts=cfg.gateLocations.map(x=>x-100),expand=x=>x+1300*cuts.filter(c=>x>=c).length;
    ['platformSpots','movingSpots','collapsingRocks','boneOffsets','crystalOffsets','enemies','hazards','thorns'].forEach(key=>(cfg[key]||[]).forEach(item=>{
      ['x','minX','maxX'].forEach(k=>{if(typeof item[k]==='number')item[k]=expand(item[k]);});
    }));
    cfg.trenches.forEach(t=>{t.startX=expand(t.startX);t.endX=expand(t.endX);});
    cfg.gateLocations=cfg.gateLocations.map(expand);cfg.exitX=expand(cfg.exitX);cfg.levelWidth=15200;
    art.sections.forEach(section=>{section.landmark[1]=expand(section.landmark[1]);section.decor.forEach(item=>item[1]=expand(item[1]));});
    ['distant','groundPatches','ceiling'].forEach(key=>art[key].forEach(item=>item[1]=expand(item[1])));
    cfg.cutterSpots=[];cfg.cannonSpots=[];cfg.sweepSpots=[];cfg.timingTrials=[];
    cuts.forEach((cut,i)=>{
      const x=cut+i*1300;
      cfg.cutterSpots.push(x+240);
      art.sections[i].decor.push(['rock_spire',x+100,0.62],['crystal_cluster',x+490,1.25],['rock_stack',x+1110,1.2]);
      art.groundPatches.push(['ground_cavern',x+300,0.8],['ground_5',x+1000,0.8]);
      art.ceiling.push(['rock_peak',x+180,0.6,-460],['rock_stack',x+730,1.6,-450],['rock_spire',x+1150,0.65,-440]);
      if([1,3,5,6].includes(i)){
        const width=i===6?280:260;
        cfg.trenches.push({startX:x+600,endX:x+600+width});
        cfg.platformSpots.push({x:x+665,y:-80,scale:0.22},{x:x+800,y:-80,scale:0.22});
        cfg.movingSpots.push({x:x+730,y:-115,distanceX:55,duration:2150,scale:0.35});
        if(i>=5)cfg.timingTrials.push({startX:x+600,endX:x+600+width,x:x+735,period:5200,closed:i===6?3500:3200,label:i===6?'CRYSTAL CORE GAUNTLET':'AMETHYST LOCK'});
      }else{
        cfg.thorns.push({x:x+680,y:0,scale:0.8,damage:8});
        cfg.platformSpots.push({x:x+680,y:-130,scale:0.35});
        cfg.movingSpots.push({x:x+880,y:-85,distanceY:-100,duration:2350,scale:0.35});
      }
      if(i>=5){
        cfg.cutterSpots.push(x+1060);
        cfg.cannonSpots.push({x:x+560,y:-52,minX:x+70,maxX:x+590,period:1150},{x:x+1120,y:-165,minX:x+900,maxX:x+1140,period:1450});
        cfg.sweepSpots.push({x:x+420,y:-205,range:100});
        cfg.enemies.push({type:'ground',x:x+160,y:-45,minX:x+80,maxX:x+280,speed:100},
          {type:'fly',x:x+480,y:-180,minX:x+390,maxX:x+560,speed:95},
          {type:'armored',x:x+1000,y:-45,minX:x+930,maxX:x+1130,speed:80});
      }
      LEVEL_SECTIONS[2][i][2]=art.sections[i].landmark[1];
    });
    cfg.trenches.sort((a,b)=>a.startX-b.startX);
  }
  // Starlight Summit: long open-air routes, wind lifts and final sky trials.
  {
    const cfg=LEVEL_CONFIGS[3],art=window.CosmicIllustratedScenes[3];
    const cuts=cfg.gateLocations.map(x=>x-100),expand=x=>x+1400*cuts.filter(c=>x>=c).length;
    ['platformSpots','movingSpots','collapsingRocks','boneOffsets','crystalOffsets','enemies','hazards','thorns'].forEach(key=>(cfg[key]||[]).forEach(item=>{
      ['x','minX','maxX'].forEach(k=>{if(typeof item[k]==='number')item[k]=expand(item[k]);});
    }));
    cfg.trenches.forEach(t=>{t.startX=expand(t.startX);t.endX=expand(t.endX);});
    cfg.gateLocations=cfg.gateLocations.map(expand);cfg.exitX=expand(cfg.exitX);cfg.levelWidth=16000;
    art.sections.forEach(section=>{section.landmark[1]=expand(section.landmark[1]);section.decor.forEach(item=>item[1]=expand(item[1]));});
    ['distant','groundPatches'].forEach(key=>art[key].forEach(item=>item[1]=expand(item[1])));
    cfg.cutterSpots=[];cfg.cannonSpots=[];cfg.sweepSpots=[];cfg.timingTrials=[];
    cuts.forEach((cut,i)=>{
      const x=cut+i*1400;
      cfg.cutterSpots.push(x+240);
      art.sections[i].decor.push(['rock_spire',x+100,0.62],['rune_pillar',x+490,1.5],['rock_stack',x+1110,1.2]);
      art.groundPatches.push(['ground_5',x+300,0.8],['ground_5',x+1000,0.8]);
      if([1,3,5,6].includes(i)){
        const width=i===6?300:280;
        cfg.trenches.push({startX:x+600,endX:x+600+width});
        cfg.platformSpots.push({x:x+665,y:-80,scale:0.22},{x:x+800,y:-80,scale:0.22});
        cfg.movingSpots.push({x:x+730,y:-115,distanceX:55,duration:2050,scale:0.35});
        if(i>=5)cfg.timingTrials.push({startX:x+600,endX:x+600+width,x:x+735,period:5200,closed:i===6?3550:3300,label:i===6?'CELESTIAL FINAL TRIAL':'SKYBRIDGE LOCK'});
      }else{
        cfg.thorns.push({x:x+680,y:0,scale:0.8,damage:8});
        cfg.platformSpots.push({x:x+680,y:-130,scale:0.35});
        cfg.movingSpots.push({x:x+880,y:-85,distanceY:-100,duration:2250,scale:0.35});
      }
      if(i===2||i===4)cfg.hazards.push({type:'wind',minX:x+420,maxX:x+960,forceX:45});
      if(i>=5){
        cfg.cutterSpots.push(x+1060);
        cfg.cannonSpots.push({x:x+560,y:-52,minX:x+70,maxX:x+590,period:1100},{x:x+1120,y:-165,minX:x+900,maxX:x+1140,period:1400});
        cfg.sweepSpots.push({x:x+420,y:-205,range:100});
        cfg.enemies.push({type:'ground',x:x+160,y:-45,minX:x+80,maxX:x+280,speed:100},
          {type:'fly',x:x+480,y:-180,minX:x+390,maxX:x+560,speed:95},
          {type:'armored',x:x+1000,y:-45,minX:x+930,maxX:x+1130,speed:80});
      }
      LEVEL_SECTIONS[3][i][2]=art.sections[i].landmark[1];
    });
    cfg.trenches.sort((a,b)=>a.startX-b.startX);
  }
  ...
  class AdventureLevelScene extends Phaser.Scene {
    ...
    createCannonEncounters() {
      ...
      this.cannonShots=[];
      for(let i=0;i<24;i++){
        const shot=this.add.circle(0,0,9,0xffb44b).setStrokeStyle(2,0xfff0af).setDepth(64).setVisible(false);
        this.physics.add.existing(shot);shot.body.setAllowGravity(false);shot.body.enable=false;shot.damage=8;shot.hazardLabel='Cannon shot';
        this.physics.add.overlap(this.dog,shot,(dog,h)=>{this.handleDogHazardCollision(dog,h);h.live=false;h.body.enable=false;h.setVisible(false);});
        this.cannonShots.push(shot);
      }
      this.cannons=this.levelConfig.cannonSpots.map((cfg,i)=>{
        ...
        return {...cfg,y,light,clock:i*270};
      });
      this.sweepCutters=(this.levelConfig.sweepSpots?this.levelConfig.sweepSpots.map(c=>({...c,y:g+c.y})):[{x:9830,y:g-200,range:95},{x:12960,y:g-210,range:120}]).map(cfg=>{
        this.add.rectangle(cfg.x,cfg.y,cfg.range*2,5,0x738779).setDepth(31);
        const blade=this.physics.add.image(cfg.x,cfg.y,'woodland-cutter').setDepth(65);
        blade.body.setAllowGravity(false).setImmovable(true).setCircle(28,12,12);blade.damage=8;blade.hazardLabel='Sweeping cutter';
        this.physics.add.overlap(this.dog,blade,(dog,h)=>this.handleDogHazardCollision(dog,h));
        return {...cfg,blade};
      });
    }

    updateCannonEncounters(delta) {
      this.cannons.forEach(c=>{
        if(this.dog.x<c.minX-250||this.dog.x>c.maxX+250)return;
        c.clock+=delta;c.light.setAlpha(c.clock%c.period>c.period-350?1:0.15);
        if(c.clock>=c.period){
          c.clock%=c.period;const shot=this.cannonShots.find(p=>!p.live);
          if(shot){shot.live=true;shot.minX=c.minX;shot.body.enable=true;shot.body.reset(c.x-70,c.y);shot.setVisible(true);}
        }
      });
      this.cannonShots.forEach(p=>{
        if(!p.live)return;p.body.reset(p.x-delta*(this.levelConfig.id>=2?0.35:0.32),p.y);
        if(p.x<p.minX){p.live=false;p.body.enable=false;p.setVisible(false);}
      });
      this.sweepCutters.forEach((c,i)=>{c.blade.body.reset(c.x+Math.sin(this.woodlandTrapClock/600+i)*c.range,c.y);c.blade.rotation+=delta*0.01;});
    }

    createWoodlandTraps() {
      const g=this.groundY;
      if(!this.textures.exists('woodland-cutter')){
        const art=this.make.graphics({x:0,y:0,add:false});
        art.fillStyle(0xb7c4bd);art.lineStyle(3,0x394941);
        const points=[];
        for(let i=0;i<32;i++){const a=i*Math.PI/16,r=i%2?25:36;points.push({x:40+Math.cos(a)*r,y:40+Math.sin(a)*r});}
        art.fillPoints(points,true);art.strokePoints(points,true);art.fillStyle(0x566b60);art.fillCircle(40,40,16);
        art.fillStyle(0xffcb74);art.fillCircle(40,40,6);art.generateTexture('woodland-cutter',80,80);art.destroy();
      }
      this.timingTrials=(this.levelConfig.timingTrials||[]).map(cfg=>{
        const wall=this.add.rectangle(cfg.x,g-205,42,530,0xffb64f,0.28).setStrokeStyle(3,0xffdc85).setDepth(59);
        this.physics.add.existing(wall);wall.body.setAllowGravity(false).setImmovable(true);
        wall.damage=8;wall.hazardLabel='Clockwork barrier';
        this.physics.add.collider(this.dog,wall,(dog,h)=>this.handleDogHazardCollision(dog,h));
        const blades=[-400,-290,-180,-70].map(dy=>this.add.image(cfg.x,g+dy,'woodland-cutter').setDepth(65));
        const sign=this.add.text(cfg.startX-115,g-185,cfg.label+'\nWAIT',{
          fontFamily:'sans-serif',fontSize:'17px',fontStyle:'bold',color:'#ffdf92',backgroundColor:'#253830',padding:{x:9,y:7},align:'center'
        }).setOrigin(0.5).setDepth(75);
        return {...cfg,wall,blades,sign,clock:0,open:false};
      });
      ...
      this.levelConfig.cutterSpots.forEach((x,i)=>{
        ...
        blade.body.setAllowGravity(false).setImmovable(true).setCircle(28,12,12);
        blade.damage=8;blade.hazardLabel='Moving cutter';
        this.physics.add.overlap(this.dog,blade,(dog,h)=>this.handleDogHazardCollision(dog,h));
        this.woodlandTraps.push({kind:'cutter',x,warning,sprite:blade,offset:i*1100,period:4800});
      });
    }

    updateWoodlandTraps(delta) {
      this.timingTrials.forEach(t=>{
        if(this.dog.x<t.startX-400||this.dog.x>t.endX+500)return;
        t.clock+=delta;const phase=t.clock%t.period;
        t.open=phase>=t.closed;
        t.wall.body.enable=!t.open;t.wall.setVisible(!t.open);
        t.blades.forEach(blade=>{blade.setVisible(!t.open);blade.rotation+=delta*0.007;});
        const remaining=((t.open?t.period:t.closed)-phase)/1000;
        t.sign.setText(t.label+'\n'+(t.open?'JUMP! ':'WAIT ')+remaining.toFixed(1)+'s');
        t.sign.setColor(t.open?'#94eaff':'#ffdf92');
        t.wall.setAlpha(phase>t.closed-700?0.45+Math.sin(phase/70)*0.25:0.6);
      });
      this.woodlandTrapClock+=delta;
      this.woodlandTraps.forEach(t=>{
        const phase=(this.woodlandTrapClock+t.offset)%t.period,g=this.groundY;
        if(t.kind==='rift'){
          const trial=this.timingTrials.find(z=>t.x>=z.startX&&t.x<=z.endX);
          const firing=(!trial||!trial.open)&&phase>=1000&&phase<1900;
          t.warning.setAlpha(phase<1000?0.35+0.3*Math.sin(phase/90):0.12);
          t.sprite.setVisible(firing);t.sprite.body.enable=firing;
          if(firing)t.sprite.body.reset(t.x,g+180-(phase-1000)*0.7);
        }else{
          // Long raised rest, visible warning, descend, hold, then retract.
          const y=phase<1800?g-260:phase<2400?g-260+(phase-1800)/600*220:
            phase<3200?g-40:phase<3900?g-40-(phase-3200)/700*220:g-260;
          t.sprite.body.reset(t.x,y);t.sprite.rotation+=delta*0.009;
          t.warning.setAlpha(phase>=1100&&phase<2400?0.5+0.25*Math.sin(phase/90):0.12);
        }
      });
    }
    ...
    createPaintedGround(cfg,seg,groundY) {
      ...
      if(cfg.id===1){
        ...
        [[seg.startX,true],[seg.endX,false]].forEach(([x,flip])=>{
          ...
          this.add.image(x+(flip?62:-62),groundY+90,'rock_low').setOrigin(0.5,1)
            .setScale(0.55).setFlipX(flip).setDepth(24).setMask(this.paintedBankMask);
    ...
    createIllustratedLandscape(cfg,groundY) {
      ...
      if(cfg.id===3){
        [[180,60,1.6],[1540,160,2],[2890,100,1.7],[4250,160,2.2],[5700,90,1.8],[7300,140,1.9],[9100,120,2.1],[10900,160,1.8],[12800,100,2.0],[14800,140,2.2]].forEach(([x,dy,scale])=>{
    ...
    handleDogHazardCollision(dog, hazard) {
      ...
      if (window.setSparkyMessage) {
        window.setSparkyMessage(`⚠️ <strong>${hazard.hazardLabel || "Cosmic Rock"} impact!</strong> Lost ${dmg} Energy. Watch the warning and wait for a clear path.`);
      }
    }
