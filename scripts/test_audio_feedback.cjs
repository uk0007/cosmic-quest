...
(async()=>{
 ...
 const pcm=await p.evaluate(async method=>{
  const s=new window.Sound.constructor();s.ctx=new OfflineAudioContext(1,44100*3,44100);s.sfxGain=s.ctx.createGain();s.sfxGain.gain.value=.32;s.sfxGain.connect(s.ctx.destination);s.bgmGain=s.ctx.createGain();if(method==='playCorrect')s.applauseBuffer=await s.ctx.decodeAudioData(await (await fetch('assets/audio/correct-applause.mp3')).arrayBuffer());s[method]();const rendered=await s.ctx.startRendering();return Array.from(rendered.getChannelData(0));
 },method);
