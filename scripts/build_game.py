...
<head>
  ...
  <style>
    ...
    * {
    ...
    .adv-gate-footer {
      flex-shrink: 0;
      padding: 10px max(20px, env(safe-area-inset-right)) max(12px, env(safe-area-inset-bottom)) max(20px, env(safe-area-inset-left));
      background: rgba(15, 23, 42, 0.96);
    ...
    /* OPTION B: Golden Amber */
    .adv-gate-option-btn.opt-b {
      background: linear-gradient(135deg, rgba(180, 125, 15, 0.22), rgba(234, 179, 8, 0.08));
      border-color: rgba(250, 204, 21, 0.5);
      color: #fffbeb;
    }
    ...
    .adv-gate-option-btn.opt-b:hover:not(:disabled) {
      border-color: #facc15;
      background: linear-gradient(135deg, rgba(180, 125, 15, 0.42), rgba(234, 179, 8, 0.2));
    ...
    .adv-gate-option-btn.opt-d {
      background: linear-gradient(135deg, rgba(129, 140, 248, 0.24), rgba(199, 210, 254, 0.12));
      border-color: #818cf8;
      color: #eef2ff;
    }
    ...
    .adv-gate-option-btn.opt-d:hover:not(:disabled) {
      border-color: #c7d2fe;
      background: linear-gradient(135deg, rgba(129, 140, 248, 0.45), rgba(199, 210, 254, 0.24));
    ...
    /* Standalone / PWA App Mode & Full-Screen Layout */
    body.is-standalone,
    body:has(#screen-welcome.active),
    body:has(#screen-adventure.active),
    body:has(#screen-adventure-victory.active),
    body.adventure-mode-active {
      margin: 0 !important;
      padding: 0 !important;
    ...
    body.is-standalone .app-container,
    ...
    /* In installed standalone mode, hide install bars and guidance */
    body.is-standalone .title-install-bar,
    body.is-standalone #row-settings-install {
      display: none !important;
    }

    @media (display-mode: standalone), (display-mode: fullscreen) {
      .title-install-bar,
      #row-settings-install {
        display: none !important;
      }
    }
...
<body>
    ...
    <section class="screen active" id="screen-welcome">
      ...
      <div class="title-screen-container">
        ...
        <div class="title-install-bar" id="title-install-bar" style="display: none;">
          <span class="title-install-text" id="title-install-text">📲 Install Cosmic Quest for Full-Screen Play</span>
  ...
  <script>
    ...
    class UpbeatAudioEngine {
      ...
      tone(freq, at, duration, volume, type='triangle', bus=this.sfxGain, endFreq=null) {
        ...
        gain.gain.setValueAtTime(0.001,at);gain.gain.linearRampToValueAtTime(volume,at+0.008);
        gain.gain.exponentialRampToValueAtTime(0.001,at+duration);
        osc.connect(gain);gain.connect(bus);osc.start(at);osc.stop(at+duration+0.02);
        osc.onended=()=>{osc.disconnect();gain.disconnect();};
      }

      noise(at,duration,volume,frequency,bus=this.sfxGain) {
        if(!this.noiseBuffer){
        ...
        src.buffer=this.noiseBuffer;filter.type='bandpass';filter.frequency.value=frequency;filter.Q.value=0.8;
        gain.gain.setValueAtTime(0.001,at);gain.gain.linearRampToValueAtTime(volume,at+0.004);
        gain.gain.exponentialRampToValueAtTime(0.001,at+duration);
        src.connect(filter);filter.connect(gain);gain.connect(bus);src.start(at);src.stop(at+duration+0.01);
        src.onended=()=>{src.disconnect();filter.disconnect();gain.disconnect();};
      }

      duckMusic(duration) {
        const now=this.ctx.currentTime,g=this.bgmGain.gain;
        g.cancelScheduledValues(now);g.setValueAtTime(g.value,now);
        g.linearRampToValueAtTime(0.045,now+0.04);
        g.setValueAtTime(0.045,now+duration);g.linearRampToValueAtTime(0.16,now+duration+0.35);
      }

      /* Bright C–G–Am–F groove: kick, backbeat, shaker, bass and mallet melody. */
      startUpbeatMusic() {
        if(!this.ctx||this.bgmInterval||!this.bgmEnabled)return;
        this.bgmGain.gain.cancelScheduledValues(this.ctx.currentTime);
        this.bgmGain.gain.setValueAtTime(0.16,this.ctx.currentTime);
        const stepDuration=60/this.bpm/4;
        let next=this.ctx.currentTime+0.04;
        ...
        const melody=[523.25,0,659.25,783.99,0,659.25,587.33,0,523.25,659.25,0,880,783.99,0,659.25,0,
          587.33,0,783.99,987.77,0,783.99,659.25,0,587.33,0,659.25,783.99,0,587.33,493.88,0];
        const tick=()=>{
          if(next<this.ctx.currentTime-0.2)next=this.ctx.currentTime+0.03;
          while(next<this.ctx.currentTime+0.12){
            const step=this.step++%128,beat=step%16,bar=Math.floor(step/16),root=roots[bar%4];
            if(beat%4===0)this.tone(135,next,0.18,0.62,'sine',this.bgmGain,42);
            if(beat===4||beat===12){this.noise(next,0.11,0.24,1600,this.bgmGain);this.tone(185,next,0.09,0.15,'triangle',this.bgmGain);}
            if(beat%2===0)this.noise(next,0.035,beat%4===0?0.09:0.14,7200,this.bgmGain);
            if([0,6,8,14].includes(beat))this.tone(root*(beat===14?1.5:1),next,0.21,0.32,'triangle',this.bgmGain);
            if(beat===2||beat===10)[1,bar%4===2?1.2:1.25,1.5].forEach(r=>this.tone(root*2*r,next,0.18,0.09,'triangle',this.bgmGain));
            const note=melody[step%32];if(note)this.tone(note,next,0.16,bar>=4?0.16:0.12,'sine',this.bgmGain);
            next+=stepDuration;
          }
        };
        tick();this.bgmInterval=setInterval(tick,25);
      }
      ...
      stopUpbeatMusic() {
        ...
        if(this.ctx&&this.bgmGain){this.bgmGain.gain.cancelScheduledValues(this.ctx.currentTime);this.bgmGain.gain.setTargetAtTime(0,this.ctx.currentTime,0.02);}
      }
      ...
      playWrong() {
        if(!this.sfxEnabled||!this.ctx)return;
        const now=this.ctx.currentTime;this.duckMusic(0.85);
        [349.23,311.13,261.63].forEach((f,i)=>this.tone(f,now+i*0.17,0.22,0.25,'triangle',this.sfxGain,f*0.94));
        this.tone(220,now+0.5,0.38,0.24,'triangle',this.sfxGain,95);
        this.tone(100,now+0.82,0.16,0.13,'sine',this.sfxGain,155);
      }
    ...
    if (toggleMusicBtn) {
      toggleMusicBtn.addEventListener('click', () => {
        ...
        Sound.playClick();
        Sound.toggleBgm();
        toggleMusicBtn.textContent = Sound.bgmEnabled ? 'ON' : 'OFF';
        toggleMusicBtn.classList.toggle('off', !Sound.bgmEnabled);
        if (!Sound.bgmEnabled) {
          Sound.stopUpbeatMusic();
        } else {
          Sound.startUpbeatMusic();
        }
      });
    }
    ...
    // Standalone / PWA Mode Detection Helper
    ...
    // iOS Safari Detection Helper (supports iPhones, iPods, and modern iPads with MacIntel UA)
    function isIosSafari() {
      ...
      const isOtherBrowser = /CriOS|FxiOS|OPiOS|EdgiOS|mercury/i.test(ua);
      return isIOS && isWebKit && !isOtherBrowser;
    }
    window.isIosSafari = isIosSafari;

    // Check & display install guidance for iOS Safari or Android
    function updateInstallPromptVisibility() {
      if (isStandaloneMode() || localStorage.getItem('cq_install_dismissed')) {
        const titleBar = document.getElementById('title-install-bar');
        if (titleBar) titleBar.style.display = 'none';
        return;
      }
      ...
      const titleBar = document.getElementById('title-install-bar');
      const textEl = document.getElementById('title-install-text') || (titleBar && titleBar.querySelector('span'));
      const installBtn = document.getElementById('btn-title-install');
      const settingsRow = document.getElementById('row-settings-install');

      if (isIosSafari()) {
        if (titleBar && textEl) {
          textEl.innerHTML = '📲 Play Full-Screen: Tap <strong>Share ⎋</strong> → <strong>Add to Home Screen ➕</strong>';
          if (installBtn) installBtn.style.display = 'none';
          titleBar.style.display = 'inline-flex';
        }
        if (settingsRow) settingsRow.style.display = 'flex';
      } else if (deferredInstallPrompt) {
        if (titleBar && textEl) {
          textEl.textContent = '📲 Install Cosmic Quest for Full-Screen Play';
    ...
    // Sync on media query changes
    if (window.matchMedia) {
      const mqStandalone = window.matchMedia('(display-mode: standalone)');
      if (mqStandalone.addEventListener) mqStandalone.addEventListener('change', syncStandaloneModeClass);
      const mqFullscreen = window.matchMedia('(display-mode: fullscreen)');
      if (mqFullscreen.addEventListener) mqFullscreen.addEventListener('change', syncStandaloneModeClass);
    }
    syncStandaloneModeClass();
