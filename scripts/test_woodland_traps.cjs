...
(async()=>{
 ...
 await page.evaluate(()=>CosmicAdventureEngine.startAdventure(1));await page.waitForFunction(()=>window.currentAdventureScene?.woodlandTraps?.length===5);
 const result=await page.evaluate(()=>{
  const s=currentAdventureScene; s.physics.pause();
  const r=s.woodlandTraps[0],c=s.woodlandTraps[3];
 ...
 assert.equal(result.width,7300);assert(!result.warning.shot&&result.burst.shot&&!result.safe.shot);assert(result.lowered.cutterY-result.raised.cutterY>200);assert.equal(result.retracted.cutterY,result.raised.cutterY);assert(result.clearances.every(x=>x>=300));
 for(const [name,x] of [['gap',2470],['cutters',5200]]){
  ...
  await page.waitForTimeout(100);await page.screenshot({path:`artifacts/level1-traps/${name}.png`});
 }
 assert.deepEqual(errors,[]);fs.writeFileSync('artifacts/level1-traps/trap-checks.json',JSON.stringify({result,errors},null,2));console.log(result);await browser.close();
